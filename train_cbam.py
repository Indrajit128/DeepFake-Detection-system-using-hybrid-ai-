"""
Train the CNN + CBAM attention model.
...
def main():
    ...
    model = compile_cbam_cnn(model)
    model.summary()

    # 3. Callbacks
    os.makedirs(config.MODELS_DIR, exist_ok=True)
    ckpt_path = os.path.join(config.MODELS_DIR, "cbam_best.keras")
    callbacks = [
    ...
    plot_confusion_matrix(y_test, y_pred, "CNN_CBAM")
